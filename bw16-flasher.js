...
class RealtekSerial {
  ...
  async waitFor(matches, timeout = 5000, expected = 'the BW16 bootloader') {
  ...
  async waitForNak() { return this.waitFor((byte) => byte === NAK, 5000, 'the BW16 ready signal'); }
  async waitForAck() { return this.waitFor((byte) => byte === ACK, 5000, 'the BW16 packet acknowledgement'); }
  ...
  async writeBlock(address, source, sequence, onProgress, waitForStart = true) {
    const blocks = Math.ceil(source.length / 1024);
    const image = new Uint8Array(blocks * 1024).fill(0xff);
    image.set(source);
    // The ROM loader asks for a NAK before its first packet. After CMD_XMODEM
    // the flashloader sends one NAK for the entire three-image transfer, not
    // a fresh one at each image boundary.
    if (waitForStart) await this.waitForNak();
    for (let block = 0; block < blocks; block += 1) {
      ...
      // Realtek's uploader uses an 8-bit sum with an initial 0xFF value.
      // Starting at zero makes the flashloader reject a packet with NAK.
      let checksum = 0xff;
